...
generate_test_data <- function(n_obs, n_groups, overlap_ratio = 0.8) {
  ...
  y_only_indices <- indices[(n_overlap + n_x_only + 1):(n_overlap + n_x_only + n_y_only)]
...
compare_groups_anti_join <- function(x, y, group_cols) {
  x_groups <- x |> dplyr::select({{ group_cols }}) |> dplyr::distinct() |> dplyr::mutate(in_x = TRUE, in_y = FALSE)
  y_groups <- y |> dplyr::select({{ group_cols }}) |> dplyr::distinct() |> dplyr::mutate(in_x = FALSE, in_y = TRUE)
  
  # Find groups only in x (not in y)
  x_only <- dplyr::anti_join(x_groups, y_groups, by = names(x_groups)[!names(x_groups) %in% c("in_x", "in_y")])
  
  # Find groups only in y (not in x)
  y_only <- dplyr::anti_join(y_groups, x_groups, by = names(y_groups)[!names(y_groups) %in% c("in_x", "in_y")])
  
  # Combine results
  dplyr::bind_rows(x_only, y_only) |>
    dplyr::arrange(dplyr::pick(names(x_groups)[!names(x_groups) %in% c("in_x", "in_y")]))
}
...
run_benchmarks <- function() {
  ...
  for (size in sizes) {
    ...
    bm <- bench::mark(
      ...
      anti_join = compare_groups_anti_join(test_data$x, test_data$y, c(group1, group2)),
      iterations = 10,
...
for (size in unique(summary_data$size)) {
  ...
  cat(sprintf("Size %d: anti_join is %.2fx %s than full_join\n", 
              size, 
              speedup, 
