...
compare_diff <- function(data, context = c(3L,3L), max_differences = Inf) {
  ...
  for (column in compare_columns) {
    diff_mask <- diff_mask | !is_equal(data[[paste0(column, ".ours")]], data[[paste0(column, ".theirs")]])
  }
...
f_green <- formatter("span", style = "color:green; white-space: nowrap; display: block; overflow: clip; max-width: 200px")
f_red <- formatter("span", style = "color:red; white-space: nowrap; display: block; overflow: clip; max-width: 200px")
f_ctx <- formatter("span", style = "white-space: nowrap; display: block; overflow: clip; max-width: 200px")
...
show_diff <- function(diffs) {
  ...
  diffs <- diffs |>
    ...
    mutate(across(!c(.type, .side), function(x) {
      case_when(
        ...
        .side == "theirs" & !is_equal(x, lag(x)) | .type == "theirs" ~ f_green(x),
        TRUE ~ f_ctx(x)
      )
    })) |>
    ungroup() |>
  ...
  tbl <- formattable(diffs) |>
    ...
    column_spec(1:ncol(diffs), border_left = "1px solid #eeeeee", border_right = "1px solid #eeeeee") |>
...
x <- tibble(
  ...
  d2atetimes = lubridate::as_datetime("2024-01-01 12:00:00") + c(0, 1200, 2400, 3600),
  l3gl = c(TRUE, TRUE, FALSE, FALSE),
  i3nt = c(1L, 2L, 3L, 4L),
  d3bl = c(5.2, 1.2, 3.4, 5.9),
  c3hr = c("T", "H", "m", "a"),
  d3ates = lubridate::as_date("2024-01-01") + 0:3,
  d3atetimes = lubridate::as_datetime("2024-01-01 12:00:00") + c(0, 1200, 2400, 3600)
)
